...
1. At a project/session boundary read [`docs/product-core-alpha.md`](docs/product-core-alpha.md); during an active stage reread only its task, direct dependencies, and contract sections. Implement only work within the current Product Core Alpha scope.
...
- Persist an intent before every external side effect; use idempotency, fencing, reconciliation, and durable outbox patterns from the Product Core Alpha contracts.
