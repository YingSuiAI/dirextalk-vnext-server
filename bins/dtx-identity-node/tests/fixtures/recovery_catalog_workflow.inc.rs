...
async fn completion_http_fixture_accepts_exact_post_replay_and_readback() -> Result<(), Box<dyn Error>> {
    ...
    let outer_idempotency = Sha256Digest::hash_domain(b"dirextalk.history-recovery.completion-idempotency.v2\0", b"completion-invalid-outer-leafset");
    let mut outer_input = input.clone();
    outer_input.idempotency_digest = *outer_idempotency.as_bytes();
    let outer_command = history_testkit::completion_command_v2(&outer_input, &candidate);
    let CanonicalValue::Map(mut outer_fields) = decode_deterministic_cbor(&outer_command)? else { unreachable!() };
    outer_fields[14].1 = CanonicalValue::Bytes([99; 32].to_vec());
    let outer_command = encode_deterministic_cbor(&CanonicalValue::Map(outer_fields))?;
    assert!(dtx_identity_persistence::HistoryRecoveryCompletionCommand::parse(outer_command.clone(), outer_idempotency).is_err());
    let invalid = app.clone().oneshot(Request::builder().method("POST").uri(HISTORY_RECOVERY_COMPLETION_PATH.replace("{completion_id}", &completion_id.to_string())).header(header::CONTENT_TYPE, HISTORY_RECOVERY_COMPLETION_CONTENT_TYPE).header("idempotency-key", "completion-invalid-outer-leafset").header(header::AUTHORIZATION, super::authorization(&candidate_session)).body(Body::from(outer_command))?).await?;
    assert_eq!(invalid.status(), StatusCode::UNPROCESSABLE_ENTITY);
    assert!(dtx_identity_persistence::HistoryRecoveryCompletionRepository.current_descriptor(&store).await?.is_none());
    let completion_count: i64 = sqlx::query_scalar("SELECT count(*) FROM identity.history_recovery_completions_v2 WHERE identity_id=$1").bind(identity.to_string()).fetch_one(harness.identity_runtime_pool()).await?;
    assert_eq!(completion_count, 0);
