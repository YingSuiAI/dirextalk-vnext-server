...
docker compose --project-name dirextalk-vnext-production --env-file "$env_file" -f "$compose_file" up --force-recreate --no-deps --abort-on-container-failure node-ready realtime-ready opaque-push-ready
