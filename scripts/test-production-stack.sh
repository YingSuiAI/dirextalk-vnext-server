...
grep -q -- '--runtime-image dirextalk/vnet-server@sha256:<64-hex>' "$root/scripts/check-release-image.sh"
grep -q 'docker export --output' "$root/scripts/check-release-image.sh"
grep -q 'check-release-image.sh --runtime-image' "$root/scripts/publish-production-release.sh"
